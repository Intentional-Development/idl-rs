...
fn test_proposal_workflow_create_get_accept() {
    use idl_proposals::{Proposal, ProposalStatus, DiffOp, find_proposal, accept_proposal_safe};
    
    let (_temp, graph_path, orig_dir) = setup_test_env();
...
fn test_proposal_list_filter() {
    use idl_proposals::{Proposal, ProposalStatus, DiffOp, list_proposals};
...
fn test_proposal_reject_no_mutation() {
    use idl_proposals::{Proposal, DiffOp, find_proposal, audit_log};
    ...
    audit_log("reject", &proposal.id, "reviewer", Some("test"), Some(json!({"reason": "Not needed"}))).unwrap();
...
fn test_concurrent_accept_safety() {
    use idl_proposals::{Proposal, DiffOp, accept_proposal_safe, generate_proposal_id, locate_changes_dir};
    use std::sync::Arc;
    use std::thread;
    ...
    let handles: Vec<_> = proposal_ids
        ...
        .map(|id| {
            thread::spawn(move || {
                accept_proposal_safe(&id, "concurrent-reviewer", Some("test"))
            })
        })
        .collect();
    
    let results: Vec<Result<String, anyhow::Error>> = handles.into_iter().map(|h| h.join().unwrap()).collect();
...
fn test_audit_log_source_attribution() {
    ...
    let (_temp, _graph_path, orig_dir) = setup_test_env();
